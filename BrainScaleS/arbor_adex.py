...
class SingleRecipe(arbor.recipe):
    ...
    def probes(self, gid):
        ...
        return [arbor.cable_probe_membrane_voltage('"center"'),
                ...
                arbor.cable_probe_density_state('"center"', "adex", "refractory_counter"),
                arbor.cable_probe_density_state('"center"', "adex", "holdoff_counter"),
                arbor.cable_probe_density_state('"center"', "adex", "g"),
                arbor.cable_probe_density_state('"center"', "adex", "adex_current"),
        ]
...
def main(variant):
    ...
    handle_refractory_counter = sim.sample((0, 2), reg_sched)
    handle_holdoff_counter = sim.sample((0, 3), reg_sched)
    handle_g = sim.sample((0, 4), reg_sched)
    handle_adex_current = sim.sample((0, 5), reg_sched)
    ...
    data_refractory_counter, _ = sim.samples(handle_refractory_counter)[0]
    data_holdoff_counter, _ = sim.samples(handle_holdoff_counter)[0]
    data_g, _ = sim.samples(handle_g)[0]
    data_adex_current, _ = sim.samples(handle_adex_current)[0]
    ...
    data_stacked = numpy.column_stack(
        [data_mem[:, 0],
         ...
         data_mem[:, 1],
         data_w[:, 1],
         data_refractory_counter[:, 1],
         data_holdoff_counter[:, 1],
         data_g[:, 1],
         data_adex_current[:, 1],
        ]
    )
